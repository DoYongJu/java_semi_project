...
- 주요 기능
   - 모든 회원이 이용가능한 서비스
     1.  회원가입/ 로그인/ 수정/ 탈퇴
        - 회원가입: 아이디, 비밀번호 중복검사, 비밀번호 일치 확인
        - 로그인: 기본적인 로그인 기능, 아이디 찾기, 비밀번호 찾기
        - 수정: 비밀번호 수정
        - 탈퇴 
     2.  다이어트 명언 Home화면에 출력
     3.  칼로리(음식/운동)사전 
     4.  Bmi 계산기
   - 회원제 서비스
     1.  스케줄러(주간/ 월간) - 등록, 수정, 삭제, 성취률통계(주간/월간 성취률, 목표당 성취률), 당일 날짜를 기준으로 목표 주기와 달성여부를 반영한 checkbox View
     2.  식단/ 운동 유형 검사 및 관련 정보 제공
     3.  마이페이지 - 개인정보확인,수정,탈퇴,유형검사 결과 및 스케줄러 성취보고서 확인가능
     4.  커뮤니티(글/이미지) - 등록, 수정, 삭제, 카테고리 분류(전체,일반,전문), 게시글 검색(작성일, 내용, 작성자)
     5.  댓글(글) - 수정, 삭제
### 3.프로젝트 구현 화면
- 화면 기획: 카카오 Oven 사용([kakao Oven](https://ovenapp.io/, "kakao oven link"))
- 
### 4.느낀점
